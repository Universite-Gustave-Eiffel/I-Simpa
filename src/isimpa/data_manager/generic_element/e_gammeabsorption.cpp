    ...
    for (std::list<Element *>::iterator itfils = this->fils.begin(); itfils != this->fils.end(); itfils++) {
        ...
        if (infoEl.libelleElement != "moyenne" && infoEl.typeElement == ELEMENT_TYPE_ROW) {
            ...
            if (listel) {
                ...
                if (datalst->GetItemsLabel().size() < 2 ||
                    datalst->GetItemsLabel().at(0) != wxTRANSLATE("Uniform reflection")) {
                    (*itfils)->DeleteElementByXmlId(datalst->GetXmlId());
                    (*itfils)->AppendPropertyList("loi_diff", wxTRANSLATE("Diffusion law"),
                                                  {wxString(wxTRANSLATE("Uniform reflection")),
