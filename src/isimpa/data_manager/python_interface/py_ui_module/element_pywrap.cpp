...
Element *New_E_Python_Extension(const Element::ELEMENT_TYPE &baseType, Element *parent, const wxString &module,
                                const wxString &pyclass, boost::python::object &pyobj);
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (menuListSize == 0) {
            ...
            if (hasattr(extract_or_throw<boost::python::object>(menutuple[1]), "__len__")) {
                ...
                menu.Append(Element::GetMenuItem(&menu, -1, wxGetTranslation(extract_wxstring(menutuple[0])), submenu));
            } else {
                wxUiMenuItem *newMenu;
                if (menuListSize >= 3) //avec ou sans icone
                    newMenu = wxDynamicCast(
                        Element::GetMenuItem(&menu,extract_or_throw<int>(menutuple[1]),extract_wxstring(menutuple[0]),
                            extract_wxstring(menutuple[2])), wxUiMenuItem);
                else
                    newMenu = wxDynamicCast(
                        Element::GetMenuItem(&menu,extract_or_throw<int>(menutuple[1]),extract_wxstring(menutuple[0])),
                        wxUiMenuItem);
...
boost::python::list GetMenuItems(wxMenu *menuToFill) {
    ...
    auto curNode = menuitems.GetFirst();

    while (curNode) {
        auto *curItem = curNode->GetData();
        if (curItem) {
            if (curItem->IsSubMenu()) {
                retLst.append(boost::python::make_tuple(
                    WXSTRINGTOSTDWSTRING(curItem->GetItemLabel()), GetMenuItems(curItem->GetSubMenu())));
            } else {
                if (curItem->IsEnabled()) {
                    if (!curItem->IsSeparator()) {
                        ...
                        if (curUiItem) {
                            retLst.append(boost::python::make_tuple(
                                WXSTRINGTOSTDWSTRING(curUiItem->GetItemLabel()), curUiItem->GetId(),
                                WXSTRINGTOSTDWSTRING(curUiItem->GetBitmapPath())));
                        } else {
...
void export_project_class() {
    ...
    class_<element>("element", init<int>())
            .def(init<element>())
            .def("childs", &element::childs,
                 REMTR(
                     "Returns the indices of the elements contained in the object.\nEach element is defined by three values:\n - the first is the index of the element allowing to create the object\n - the second is its type\n - the third sound labeled\n \nThe index is unique for all elements of the project, however it is constant that on the execution instance of the software and is modified when loading the project file\nThe type corresponds to a set of similar element, this type is constant even for the next update of the software.\nThe label corresponds to the one visible in the tree of the project and is thus very variable. Only the elements of properties have a fixed name and can be used safely."))
            .def("updatestringconfig", &element::updatestringconfig,REMTR("Update a data field"),
                 args("propertyname", "propertynewvalue"))
            .def("updatedecimalconfig", &element::updatedecimalconfig,REMTR("Update a data field"),
                 args("propertyname", "propertynewvalue"))
            .def("updatelistconfig", &element::updatelistconfig,REMTR("Update a data field"),
                 args("propertyname", "propertynewindex"))
            .def("updateentierconfig", &element::updateentierconfig,REMTR("Update a data field"),
                 args("propertyname", "propertynewvalue"))
            .def("updateboolconfig", &element::updateboolconfig,REMTR("Update a data field"),
                 args("propertyname", "propertynewvalue"))
            .def("updatepositionconfig", &element::updatepositionconfig,REMTR("Update a data field"),
                 args("propertyname", "propertynewvalue"))
            .def("getstringconfig", &element::getstringconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("getpositionconfig", &element::getpositionconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("getdecimalconfig", &element::getdecimalconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("getboolconfig", &element::getboolconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("getentierconfig", &element::getentierconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("getcolorconfig", &element::getcolorconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("getlistconfig", &element::getlistconfig,REMTR("Returns the value of a data field"),
                 args("propertyname"))
            .def("appendpropertytext", &element::appendpropertytext, appendpropertytext_overloads(
                     REMTR("Adds a text field in the element."),
                     args("propertyname", "propertylabel", "propertydefaultvalue", "readonly", "exporttocore")))
            .def("appendpropertyposition", &element::appendpropertyposition, appendpropertyposition_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyname", "propertylabel", "propertydefaultvalue", "exporttocore")))
            .def("appendpropertylist", &element::appendpropertylist, appendpropertylist_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyName", "propertyLabel", "values", "defaultValue", "asTitle", "hSize",
                          "exportToCore")))
            .def("appendpropertycolor", &element::appendpropertycolor, appendpropertycolor_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyname", "propertylabel", "defaultred", "defaultgreen", "defaultblue")))
            .def("appendpropertyentier", &element::appendpropertyentier, appendpropertyentier_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyname", "propertylabel", "propertydefaultvalue", "exporttocore", "ismaxvalue",
                          "isminvalue", "maxvalue", "minvalue")))
            .def("appendpropertydecimal", &element::appendpropertydecimal, appendpropertydecimal_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyname", "propertylabel", "propertydefaultvalue", "readonly", "precision",
                          "ismaxvalue", "isminvalue", "maxvalue", "minvalue", "exporttocore")))
            .def("appendpropertybool", &element::appendpropertybool, appendpropertybool_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyname", "propertylabel", "propertydefaultvalue", "exporttocore")))
            .def("appendpropertyfont", &element::appendpropertyfont, appendpropertyfont_overloads(
                     REMTR("Adds a data field to the element."),
                     args("propertyname", "propertylabel", "propertydefaultvalue")))
            .def("getmenu", &element::getmenu,REMTR("Returns the possible actions to perform on the element."))
            .def("getallelementbytype", &element::getallelementbytype,
                 REMTR("Returns all the elements having as type that passed in parameter."), args("elementtype"))
            .def("getelementbytype", &element::getelementbytype,
                 REMTR("Returns the first element encountered of this type in child elements and recursively"),
                 args("elementtype"))
            .def("getelementbylibelle", &element::getelementbylibelle,
                 REMTR(
                     "Returns the first element encountered with this label.For the property elements this will be the name and not the label."),
                 args("elementlabel"))
            .def("getinfos", &element::getinfos,
                 REMTR("Returns the information of this element such as its label, type, etc.."))
            .def("appenduserelement", &element::appenduserelement,
                 REMTR(
                     "Adds an element to the project from a source external to the program\n This external class must inherit from uictrl::element and be in the folder UserScript / @return The instance of the python class"),
                 args("modulename", "classname"))
            .def("appendfilsbytype", &element::appendfilsbytype, appendfilsbytype_overloads(
                     REMTR(
                         "This function allows the addition of a child from its type. This function only works with compatible elements (having a parent / child link)\n @ return The index of the element added or -1 if no element added."),
                     args("element_type", "element_label")))
            .def("modified", &element::modified,REMTR("Mark the item as updated and will be saved in the xml file."))
            .def("hasproperty", &element::hasproperty,
                 REMTR("Returns true if this element has a property with this name"))
            .def("hide", &element::hide, hide_overloads("Hide an element", args("visible=False")))
            .def("setreadonlyconfig", &element::setreadonlyconfig,
                 setreadonlyconfig_overloads("Enable or Disable the user access to this data.",
                                             args("name", "readOnly", "col")))
            .def("setreadonlyallconfig", &element::setreadonlyallconfig, setreadonlyallconfig_overloads(
                     "Enables / Disables the ability to change all the fields to the user for a given column.",
                     args("readOnly", "col")))
            .def("deleteelementbyxmlid", &element::deleteelementbyxmlid, deleteelementbyxmlid_overloads(
                     "Find and delete the son with this id. This is not a recursively search",
                     args("xmlIdElement", "setModification")))
            .def("deleteallelementbytype", &element::deleteallelementbytype,
                 REMTR("Removes all children from this element at this level with this type of element."),
                 args("elementType"))
            .def("deleteallelementbytyper", &element::deleteallelementbytyper,
                 REMTR("Deletes recursively all descendants with this data type"), args("elementType"))
            .def("register_update_manager", &element::register_update_manager,
                 REMTR(
            ...
            .def("register_appendchild_manager", &element::register_appendchild_manager,
                 REMTR(
                     "Link a python function to the event of element append a new element child.You can add multiple links. Links are alive until project close."));
...
Element *GetElement(const wxInt32 &xmlId) {
    ...
    if (InstanceManager::GetReference(OBJECT_REF_TYPE_ELEMENT, xmlId, &obj)) {
        ...
        ThrowPyException(WXSTRINGTOCHARPTR(wxGetTranslation("This element doesn't exist")));
        return NULL;
    }
}
...
namespace uictrl {
    ...
    boost::python::dict element::getinfos() {
        ...
        retval["label_located"] = WXSTRINGTOSTDWSTRING(wxGetTranslation(self->GetTreeLabel()));
    ...
    bool element::updatepositionconfig(const std::wstring &name, const boost::python::list &newValue) {
        Element *self = GetElement(xmlId);
        vec3 newpos(extract_or_throw<float>(newValue[0]), extract_or_throw<float>(newValue[1]),
                    extract_or_throw<float>(newValue[2]));
    ...
    int element::appendpropertytext(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                    const std::wstring &propertyDefaultValue, bool readOnly, bool exportToCore) {
        Element *self = GetElement(xmlId);
        Element *newobj = self->AppendPropertyText(propertyName, propertyLabel, propertyDefaultValue, readOnly,
                                                   exportToCore);
        if (newobj) {
    ...
    int element::appendpropertyposition(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                        const boost::python::list &propertyDefaultValue, bool exportToCore) {
        Element *self = GetElement(xmlId);
        Element *newobj = self->AppendPropertyPosition(propertyName, propertyLabel,
                                                       vec3(extract_or_throw<float>(propertyDefaultValue[0]),
                                                            extract_or_throw<float>(propertyDefaultValue[1]),
                                                            extract_or_throw<float>(propertyDefaultValue[2])),
                                                       exportToCore);
        if (newobj) {
            self->FillWxTree();
            return newobj->GetXmlId();
    ...
    int element::appendpropertylist(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                    const boost::python::list &values, long defaultValue, bool asTitle, int hSize,
                                    bool exportToCore) {
        Element *self = GetElement(xmlId);
        /*
        [['Premier element',
        ...
        */
        ...
        if (size <= 2) {
            ...
            if (!extract_array<std::wstring>(values[0], &tmpvalues)) {
                ThrowPyException(wxGetTranslation("Uncorrect data list"));
            }
            ...
            if (size == 2) {
                if (!extract_array<int>(values[1], &wxindex)) {
                    ThrowPyException(wxGetTranslation("Uncorrect index list"));
                }
            }
        } else {
            return -1;
        ...
        Element *newobj = self->AppendPropertyList(propertyName, propertyLabel, wxvalues, defaultValue, asTitle, hSize,
                                                   wxindex, exportToCore);
        if (newobj) {
            return newobj->GetXmlId();
        } else {
            return -1;
    ...
    int element::appendpropertycolor(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                     long defaultRed, long defaultGreen, long defaultBlue) {
        Element *self = GetElement(xmlId);
    ...
    int element::appendpropertyentier(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                      int propertyDefaultValue, bool exportToCore, bool isMaxValue, bool isMinValue,
                                      int maxValue, int minValue) {
        Element *self = GetElement(xmlId);
        Element *newobj = self->AppendPropertyInteger(propertyName, propertyLabel, propertyDefaultValue, exportToCore,
    ...
    int element::appendpropertydecimal(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                       float propertyDefaultValue, bool readOnly, int precision, bool isMaxValue,
                                       bool isMinValue, float maxValue, float minValue, bool exportToCore) {
        Element *self = GetElement(xmlId);
        Element *newobj = self->AppendPropertyDecimal(propertyName, propertyLabel, propertyDefaultValue, readOnly,
                                                      precision, isMaxValue, isMinValue, maxValue, minValue,
                                                      exportToCore);
        if (newobj) {
            return newobj->GetXmlId();
        } else {
    ...
    int element::appendpropertybool(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                    bool propertyDefaultValue, bool exportToCore) {
        Element *self = GetElement(xmlId);
    ...
    int element::appendpropertyfont(const std::wstring &propertyName, const std::wstring &propertyLabel,
                                    const std::wstring &propertyDefaultValue) {
        Element *self = GetElement(xmlId);
    ...
    boost::python::object element::appenduserelement(const Element::ELEMENT_TYPE &baseType,
                                                     const std::wstring &moduleName, const std::wstring &className) {
    ...
    struct LinkBetweenElementUpdateEventAndPythonScript : public t_baseEvtCatcher {
        ...
        virtual void OnModified(const int &elid) {
            try {
                ...
                wxLogError(wxGetTranslation("Error during call of Python(TM) script on element update"));
            }
        }
    ...
    struct LinkBetweenElementAppendChildEventAndPythonScript : public t_baseEvtCatcher {
        ...
        virtual void OnNewChildElement(const int &childType, const int &parentid, const int &childid) {
            try {
                ...
                wxLogError(wxGetTranslation("Error during call of Python(TM) script on element append child"));
            }
        }
