...
const char *TranslateBoost(const std::wstring &string) {
    return wxGetTranslation(string);
}
...
void export_application_class() {
    //,args("String"),wxGetTranslation("Traduction de la chaine dans le language locale")
    def("_", TranslateBoost, args("String"),
        "Use the I-SIMPA translation dictionary and return the translated version of the parameter.");
    class_<application>("application", no_init)
            ...
            .def("getrootpreference", &application::getrootpreference,
                 "Return the element id of the user option tree root (project independent).")
            .def("getdataarray", &application::getdataarray,
                 "Return the data of a grid as shown to the user. Works with element with properties and element of GABE data type.",
                 args("element"))
            .def("loadproject", &application::loadproject,
                 "Close the current project without saving then load the I-SIMPA project file from the path parameter.",
                 arg("path"))
            ...
            .def("getuserinput", &application::getuserinput,
                 "Show a dialog with a grid, the user can update the values, then apply or cancel. Rows is a dictionnary and the dictionnary value can be a string (for simple text entry) or list for a combox box entry.",
                 args("title,msg,rows"))
            .def("register_menu_manager", &application::register_menu_manager,
                 "Register an object to manage the menu popup with the element_type.", args("element_type", "manager"))
            .def("register_event", &application::register_event,
                 "Register a function and return the event_id to set when the menu manager put the associated menu item.",
                 args("object"))
            .def("saveproject", &application::saveproject,
                 saveproject_overloads("Save the project to the current location if there is no argument.",
                                       arg("path=u''")))
            .def("sendevent", &application::sendevent,
                 sendevent_overloads("Run the specified event on the argument element.",
                                     args("element,eventid,parameters")))
            ...
            .def("getlocale", &application::getlocale,
                 "Return the user selected language corresponding to the canonical form of current locale name. Canonical form is the one that is used on UNIX systems: it is a two- or five-letter string in xx or xx_YY format, where xx is ISO 639 code of language and YY is ISO 3166 code of the country. Examples are \"en\", \"en_GB\", \"en_US\" or \"fr_FR\".")
            .def("reloadgroupsfrommodel", &application::reloadgroupsfrommodel,
                 "Destroy old group then make new group as specified in the 3D model.")
            .def("getapplicationpath", &application::getapplicationpath,
                 "Return a dictionnary with main application paths.")
            .def("dropselectionto", &application::dropselectionto,
                 "Drag and Drop selected tree items to the element in argument.", args("element"))
            .def("setselection", &application::setselection,
                 "Unselect previous selection, then select all the element passed as parameter", args("int_list"))
...
namespace uictrl {
    ...
    void application::sendevent(const element &pyel, const int &idevent, boost::python::dict parameters) {
        ...
        if (extract_or_throw<int>(parameters.attr("__len__")()) != 0) {
            ...
            if (sizedict == values.size()) {
                ...
                wxLogError(wxGetTranslation("Incorrect data types. Values and index must be characters"));
            }
        }
        pyeventlocker lckUi(&self->pyeventmode);
    ...
    bool application::importscene(const std::wstring &path, bool keepexistingfacegroup, bool docorrection,
                                  bool domeshsurface, const std::wstring &paramTetgen) {
        ProjectManager *self = GetManager();
    ...
    */
    ...
    boost::python::tuple application::getuserinput(const std::wstring &title, const std::wstring &msg,
                                                   boost::python::dict rows) {
        ProjectManager *self = GetManager();
        wxCustomEntryDialog parametersDialog(self->mainFrame, wxGetTranslation(wxString(msg)),
                                             wxGetTranslation(wxString(title)));
        ...
        for (std::vector<std::pair<object, object> >::const_iterator it = stdArray.begin();
            ...
            std::string object_classname = boost::python::extract<std::string>(
                it->second.attr("__class__").attr("__name__"));
            if (object_classname == "str") {
                ...
                if (extract_array<std::wstring, std::list<std::wstring> >(it->second, &cArrValues)) {
                    ...
                    for (std::list<std::wstring>::iterator itvals = cArrValues.begin(); itvals != cArrValues.end();
                         itvals++) {
    ...
    std::wstring application::getversion() {
        return WXSTRINGTOSTDWSTRING(
            wxString::Format("%d.%d.%d", ApplicationConfiguration::SPPS_UI_VERSION_MAJOR, ApplicationConfiguration::
                SPPS_UI_VERSION_MINOR, ApplicationConfiguration::SPPS_UI_VERSION_REVISION));
    }
