...
{
    return (Contains(leftIndex.a, rightIndex) && Contains(leftIndex.b, rightIndex) &&
        Contains(leftIndex.c, rightIndex));
}
...
bool LoadEleFile(tetrahedre** tabTetra, unsigned long& tabTetraSize, wxString tetraFilePath, vec3* tabNodes,
                 vec4 UnitizeVar)
{
    //Reset total volume
        ...
        *tabTetra = new tetrahedre[nbTetra];
        ...
        while (idTetra < nbTetra && getline(infile, line))
        {
            ...
            if (idTetra > 0 && idTetra <= nbTetra)
            {
                ...
                //Update total volume
                ApplicationConfiguration::GLOBAL_CURRENT_APPLICATION_INFORMATIONS.volScene += CalcTetraVolume(
                    tabNodes[sommets.a] / UnitizeVar.w, tabNodes[sommets.b] / UnitizeVar.w,
                    tabNodes[sommets.c] / UnitizeVar.w, tabNodes[sommets.d] / UnitizeVar.w);
...
bool CObjet3D::LoadMaillage(const std::string& facePath, const std::string& elePath, const std::string& nodePath,
                            const std::string& neighPath)
{
...
void CObjet3D::SetInternalFaceState(const formatCoreBIN::ioModel& modelExport,
                                    const formatMBIN::trimeshmodel& tetramodel)
{
    ...
    if (propertyModified)
        this->Save(
            (ApplicationConfiguration::GLOBAL_VAR.cacheFolderPath +
                ApplicationConfiguration::CONST_MODEL_SCENE_FILENAME).ToStdString());
}
            ...
            if (oldVol != this->tabVertexMaillage[tetranum].idVolume)
            {
                ...
                else
                {
                    if (InstanceManager::GetReference(OBJECT_REF_TYPE_ELEMENT, oldVol, &obj))
                    {
                        ...
                        if (elobj)
                        {
                            if (elobj->GetElementInfos().typeElement ==
                                Element::ELEMENT_TYPE_SCENE_ENCOMBREMENTS_ENCOMBREMENT_CUBOIDE || elobj->
                                GetElementInfos().typeElement == Element::ELEMENT_TYPE_SCENE_ENCOMBREMENTS_ENCOMBREMENT)
                            {
                                Element* elprop = elobj->GetElementByType(
                                    Element::ELEMENT_TYPE_SCENE_ENCOMBREMENTS_ENCOMBREMENT_RENDU);
                                if (elprop)
                                {
                            ...
                            {
                                Element* elprop = elobj->GetElementByType(
                                    Element::ELEMENT_TYPE_SCENE_VOLUMES_VOLUME_RENDU);
                                if (elprop)
                                {
...
void CObjet3D::_RenderMaillageLines(t_cutPlane cutPlane)
{
    ...
    for (long tetranum = 0; tetranum < this->tabVertexMaillageSize; tetranum++)
    {
        ...
        {
            int quantInside = 0;
            if (this->nodesMaillage[this->tabVertexMaillage[tetranum].sommets.a][cutPlane.plane] * cutPlane.sign >
                cutPlane.value)
                quantInside++;
            if (this->nodesMaillage[this->tabVertexMaillage[tetranum].sommets.b][cutPlane.plane] * cutPlane.sign >
                cutPlane.value)
                quantInside++;
            if (this->nodesMaillage[this->tabVertexMaillage[tetranum].sommets.c][cutPlane.plane] * cutPlane.sign >
                cutPlane.value)
                quantInside++;
            if (this->nodesMaillage[this->tabVertexMaillage[tetranum].sommets.d][cutPlane.plane] * cutPlane.sign >
                cutPlane.value)
                quantInside++;
            if (quantInside > 0 && quantInside < 4)
                showTetra = true;
...
void CObjet3D::ToCBINFormat(formatCoreBIN::ioModel& modelExport)
{
    ...
    t_HashElement::iterator it =
        ApplicationConfiguration::BeginRef(ApplicationConfiguration::ELEMENT_REF_TYPE_DRAWABLE);
...
bool CObjet3D::_SaveCBIN(const wxString& filename)
{
    using namespace formatCoreBIN;
...
void CObjet3D::GetTetraMesh(formatMBIN::trimeshmodel& trimesh, bool toRealCoords)
{
    ...
    if (!toRealCoords)
    {
        ...
        {
            trimesh.nodes.push_back(
                formatMBIN::t_binNode(nodesMaillage[idNode].x, nodesMaillage[idNode].y, nodesMaillage[idNode].z));
        }
    }
    else
    {
    ...
    for (int idTetra = 0; idTetra < tabVertexMaillageSize; idTetra++)
    {
        ...
        for (int idFace = 0; idFace < 4; idFace++)
        {
            trimesh.tetrahedres[idTetra].tetrafaces[idFace].marker = tabVertexMaillage[idTetra].tetrafaces[idFace].
                marker;
            trimesh.tetrahedres[idTetra].tetrafaces[idFace].vertices[0] = (int)tabVertexMaillage[idTetra].tetrafaces[
                idFace].sommets.a;
            trimesh.tetrahedres[idTetra].tetrafaces[idFace].vertices[1] = (int)tabVertexMaillage[idTetra].tetrafaces[
                idFace].sommets.b;
            trimesh.tetrahedres[idTetra].tetrafaces[idFace].vertices[2] = (int)tabVertexMaillage[idTetra].tetrafaces[
                idFace].sommets.c;
            trimesh.tetrahedres[idTetra].tetrafaces[idFace].neighbor = (int)tabVertexMaillage[idTetra].tetraNeighboor[
                idFace];
        }
    }
}

bool CObjet3D::SaveMaillage(const wxString& filename, bool toRealCoords)
{
    using namespace formatMBIN;
...
bool CObjet3D::BuildVarConstraintFile(const wxString& filename, float faceAreaContraint)
{
...
bool CObjet3D::_SavePOLY(const wxString& filename, bool exportUserModel, bool separateUserDefinedModel,
                         bool saveFaceIndex, std::vector<t_faceIndex>* fg_to_ind, bool saveFaceIndexAsGroup)
{
    ...
    for (unsigned int i = 0; i < this->_pVertices.size(); i++)
    {
        modelExport.modelVertices.push_back(
            vec3_to_dvec3(coordsOperation::GlCoordsToCommonCoords(UnitizeVar, this->_pVertices[i])));
    }
    ...
    if (exportUserModel)
    {
        t_HashElement::iterator it = ApplicationConfiguration::BeginRef(
            ApplicationConfiguration::ELEMENT_REF_TYPE_DRAWABLE);
        int eSize = GetDrawableElementSize();
        for (int navigateIndex = 0; navigateIndex < eSize; navigateIndex++)
        {
            E_Drawable* eleToDraw = dynamic_cast<E_Drawable*>(it->second);
            if (eleToDraw)
            {
                ...
                if (consistentModel.size() > 0)
                {
                    for (int idface = 0; idface < consistentModel.size(); idface++)
                    {
                        ...
                        modelExport.modelVertices.push_back(vec3_to_dvec3(
                            coordsOperation::GlCoordsToCommonCoords(UnitizeVar, consistentModel[idface].a)));
                        modelExport.modelVertices.push_back(vec3_to_dvec3(
                            coordsOperation::GlCoordsToCommonCoords(UnitizeVar, consistentModel[idface].b)));
                        modelExport.modelVertices.push_back(vec3_to_dvec3(
                            coordsOperation::GlCoordsToCommonCoords(UnitizeVar, consistentModel[idface].c)));
                        formatPOLY::t_face nvFace;
                ...
                if (eleToDraw->GetMaillageVolumeInfos(eleinfo))
                {
                    ...
                    else
                    {
                        ...
                        if (eleinfo.dotInsideVol == vec3())
                        {
                            ...
                            vec3 gTri = GetGTriangle(this->_pVertices[firstFace->Vertices.a],
                                                     this->_pVertices[firstFace->Vertices.b],
                                                     this->_pVertices[firstFace->Vertices.c]);
                            translatedDot = gTri + firstFace->FaceNormals * 0.05f;
                            elementRegion.dotInRegion = coordsOperation::GlCoordsToCommonCoords(
                                UnitizeVar, translatedDot);
                        }
                        else
                        {
                            elementRegion.dotInRegion = eleinfo.dotInsideVol;
    ...
    return polySaver.ExportPOLY(modelExport, filename.ToStdString());
}
    ...
    if (LoadNodeFile(UnitizeVar, &nodesFaces, nodesSize,WXSTRINGTOSTDSTRING(nodePath.GetFullPath())))
    {
        ...
        if (LoadFaceListe(faces, filename))
        {
            ...
            for (long f = 0; f < nbfaces; f++)
            {
                ...
                if (nodesFaces)
                    this->_pGroups[g].pFaces[f].FaceNormals = FaceNormal(
                        this->_pVertices[sommetsTri.a], this->_pVertices[sommetsTri.b], this->_pVertices[sommetsTri.c]);
