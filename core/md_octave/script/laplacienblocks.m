...
function [mat]=laplacienblocks(x,y,z,Tet,Tet_Dof,NBLOCKS,NDOF,CoeffDiff)
        ia = [];        ja = [];        s = [];
         mat = sparse ( ia, ja, s, NDOF, NDOF,12);
%            mat=zeros(nn,nn);
        k=zeros(4,1);kn=zeros(4,1);xn=zeros(4,1);yn=xn;zn=xn;
for j=1:NBLOCKS
  for  n=1:size(Tet{j},1)
             
               for l=1:4
                              kn(l)=Tet{j}(n,l);
                              k(l)=Tet_Dof{j}(n,l);
                              xn(l)=x(kn(l));  yn(l)=y(kn(l));   zn(l)=z(kn(l));
