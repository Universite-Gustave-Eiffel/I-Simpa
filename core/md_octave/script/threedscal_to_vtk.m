function threedscal_to_vtk ( x, e_conn,  AE, output_filename, title )
...
%
%    Input, real  AE(NODE_NUM,1),Acoustic Energy at each node.
%
...
%
  ...
  p = zeros ( 1, node_num );
  p(1,1:node_num) = AE(1:node_num,1)';
...
%  Write the data.
%
  vtk_AE_write ( output_unit, title, node_num, element_num, ...
    element_order, xyz, element_node, AE );

  fclose ( output_unit ) ;

  fprintf ( 1, '\n' );
  fprintf ( 1, 'THREED_TO_VTK\n' );
  fprintf ( 1, '  Les donnees sont stockees dans "%s"\n', output_filename' );
output_filename 
  return
end
