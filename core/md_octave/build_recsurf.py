...
def GetRecepteurSurfList(coreconf):
    ...
    for idrs,data in coreconf.recepteurssurf.iteritems():
        ...
        newrs.vertices=[vertB+(stepU*(idnoderow)+(stepV*(idnodecol))) for idnoderow in xrange(nbvertrow) for idnodecol in xrange(nbvertcol)]
        #Calcul des indices des sommets des faces triangulaire.
        newrs.faceindex=[[int((idcol+1+(idrow*nbvertcol))),int((idcol+(idrow*nbvertcol))),int((idcol+((idrow+1)*nbvertcol))),int((idcol+1+((idrow+1)*nbvertcol)))] for idcol in range(NbCellV) for idrow in range(NbCellU)]
