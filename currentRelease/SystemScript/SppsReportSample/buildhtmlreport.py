...
def _(msg):
    return msg
##
# If a number exist in the string, it is removed 
...
##
# @param pr_data Punctual receiver data
...
def MakePageOne( tags_dict, pr_data, sr_data ):
    ...
    if len(pr_data)!=0:
        ...
        for punctualreceiver_name, rp_data in SortedDictByKeys(pr_data):
            rplbl_lst.append(punctualreceiver_name)
    ...
    if len(pr_data)!=0:
        ...
        for punctualreceiver_name, rp_data in SortedDictByKeys(pr_data):
            ...
            for idfreq,value in enumerate(rp_data["rt30"][1:-1]):
                ...
                #print "name: %s idfreq:%i value:%f" % (punctualreceiver_name,idfreq,value)
    ...
    if display_mode=="byfreq":
        for punctualreceiver_name, rp_data in SortedDictByKeys(pr_data):
            ...
            chart.AddBars(bar_data, label=GoogleEncode(punctualreceiver_name)) #, color='ccccff'
        ...
        for punctualreceiver_name, rp_data in SortedDictByKeys(pr_data):
            ...
            rplist.append(GoogleEncode(punctualreceiver_name))
