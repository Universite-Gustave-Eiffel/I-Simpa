...
def laplacienblocks2(x, y, z, Tet_Dof, NBLOCKS, NDOF, CoeffDiff):
    ...
    ##########################
    matrice = lil_matrix(
        (NDOF + 1, NDOF + 1))  # dimension of the NDOF+1 matrix to keep the same indices as the Octave version
    ...
    for j in range(NBLOCKS):
        for n in range(len(Tet_Dof[j])):
            ...
            k = np.roll(k,
                        1)  # the vector k is shift by 1 element to the right so that the indices correspond to Octave
    ...
    return matrice[1:,
           1:].tocsr()  # a lil matrix is ​​builded but a csr matrix is ​​returned because it is faster for the rest
# ___________________________________
