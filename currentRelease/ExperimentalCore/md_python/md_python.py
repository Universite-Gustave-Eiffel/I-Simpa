...
def write_input_files(cbinpath, cmbinpath, materials, sources_lst, outfolder):
    ...
    for xmlid, mat in materials.items():
        ...
        for freqAbs in reversed(mat[
                                    "q"]):  # modifs 22/08/2024 remettre à l'endroit la matrice d'absorption. dasn le fichier XML, on vas dans l'ordre décroissant des fréquences
...
def process_output_files(outfolder, coreconf, import_data, resultsModificationLayers, nodes, tetrahedrons, w, dt):
    ...
    for idTetra in range(0, len(tetrahedrons)):
        ...
        for receiver in nearest_receivers:
            coefficient = get_a_coefficients(to_array(receiver), nodes[verts[0]], nodes[verts[1]], nodes[verts[2]],
                                             nodes[verts[3]])
            if coefficient.min() > -1e-6:
                ...
                for id_freq in range(len(coreconf.const["frequencies"])):
                    ...
                    current_frequency_id = coreconf.const["allfrequencies"].index(min(coreconf.const["allfrequencies"],
                                                                                      key=lambda x: abs(x -
                                                                                                        coreconf.const[
                                                                                                            "frequencies"][
                                                                                                            id_freq])))
                    ...
                    if receiver.isSurfReceiver:
                        ...
                        if coreconf.const["with_direct_sound"]:
                            ...
                            for source_id in steps.keys():
                                # print("5")
                                if source_id in resultsModificationLayers and receiver.idrs in \
                                        resultsModificationLayers[source_id].recsurf and len(
                                        resultsModificationLayers[source_id].recsurf[receiver.idrs][
                                            current_frequency_id]) > receiver.faceid:
                                    # print("6")
                                    power = \
                                    resultsModificationLayers[source_id].recsurf[receiver.idrs][current_frequency_id][
                                        receiver.faceid]
                        ...
                        if coreconf.const["with_direct_sound"]:
                            ...
                            for source_id in steps.keys():
                                # print("11")
                                if source_id in resultsModificationLayers and receiver.idrp in \
                                        resultsModificationLayers[source_id].recp:
                                    # print("12")
                                    power = resultsModificationLayers[source_id].recp[receiver.idrp][
                                        current_frequency_id]
                                    if power > 0 and not coreconf.const["stationary"]:
                                        # print("13")
...
def write_config_file(coreConf, outputdir):
    ...
    with open(os.path.join(outputdir, "Input_parameters.m"), "w") as f:
        ...
        f.write("SelectedFrequency=%s;\n" % [coreConf.const["allfrequencies"].index(freq) + 1 for freq in
                                             coreConf.const["frequencies"]])
...
def run_model(el, XYZ, el2Dtypd, Abs_Mater, i_nT, Wall_TL, Srce_Sonore_I, coreConf, libpath):
    ...
    nodes, tetrahedrons, w, dt = Diffusion_MD_fil_ao3.main(el, XYZ, el2Dtypd, Abs_Mater, i_nT, Wall_TL, Srce_Sonore_I,
                                                           coreConf)
...
def main(call_python=True):
    ...
    el, XYZ, el2Dtypd, Abs_Mater, i_nT, Wall_TL, Srce_Sonore_I, import_data = write_input_files(
        os.path.join(coreconf.paths["workingdirectory"], coreconf.paths["modelName"]),
        os.path.join(coreconf.paths["workingdirectory"], coreconf.paths["tetrameshFileName"]),
