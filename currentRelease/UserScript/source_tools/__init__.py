...
class manager:
    ...
    def getmenu(self,typeel,idel,menu):
        submenu=[(_(u"Activer"),self.enable_grp_sourcesid),
                 (_(u"Désactiver"),self.disable_grp_sourcesid),
                 (),
                 (_(u"Rotation"),self.rotate_grp_sourcesid),
                 (_(u"Translation"),self.translation_grp_sourcesid)]
        if len(ui.element(idel).childs())>0:
            menu.insert(2,(_(u"Toutes les sources"),submenu))
            menu.insert(2,())
        menu.insert(2,(_(u"Créer une ligne de sources sonores"),self.makelinesrcid,"Bitmaps/popup_new.png"))
        return True
    def makeline(self,idel):
        lbl_startpt=_(u"Point de départ [x,y,z]")
        lbl_nbsources=_(u"Nombre de sources")
        lbl_step=_(u"Pas [x,y,z]")
        res=ui.application.getuserinput(_(u"Créer une ligne de sources sonores"),
                                    _(u"Veuillez saisir les informations pour créer la ligne de sources sonores."),
                                    { lbl_startpt : "[0.,0.,0.]",
                                      lbl_nbsources : "1",
                                       lbl_step : "[1.,0.,0.]"
                                        })
    ...
    def rotate_src(self,idgrp):
        ...
        res=ui.application.getuserinput(_(u"Rotation d'un groupe de sources sonores"),
                                    "",
                                    { lbl_vec : "[0.,0.,1.]",
                                      lbl_angle : "90",
    ...
    def translate_src(self,idgrp):
        ...
        res=ui.application.getuserinput(_(u"Translation d'un groupe de sources sonores"),
                                    "",
                                    { lbl_vec : "[1.,0.,0.]"
                                        })
        if res[0]:
